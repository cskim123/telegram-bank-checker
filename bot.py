...
async def help_message(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if "ธนาคารที่รองรับ" in text:
        ...
        for bank, account, name, success, checked_at in rows:
            lines.append(
                f"\n{'✅' if success else '❌'} {bank} {account}\n"
                f"{name or 'ไม่พบข้อมูล'}\n{checked_at}"
            )
...
async def verify(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        if result.get("success") and result.get("fullname"):
            fullname = str(result["fullname"]).strip()
            ...
            copy_buttons = InlineKeyboardMarkup([
                ...
                [InlineKeyboardButton("🏦 คัดลอกชื่อธนาคาร",
                                      copy_text=CopyTextButton(
                                          text=BANKS.get(bank_code, bank_code)))],
            ])
            save_history(user.id, bank_code, account_number, fullname, True)
            await status.edit_text(
                "✅ ตรวจสอบสำเร็จ\n\n"
                f"ธนาคาร: {BANKS.get(bank_code, bank_code)} ({bank_code})\n"
                f"เลขบัญชี: {masked}\n"
                f"ชื่อบัญชี: {fullname}",
                reply_markup=copy_buttons,
            )
            try:
                card = make_result_card(bank_code, account_number, fullname)
                await message.reply_photo(
                    photo=card,
                    caption="🖼 รูปสรุปผลตรวจ พร้อมส่งต่อหรือบันทึกได้ครับ",
                )
            except Exception as image_error:
                logging.warning("Result card failed: %s", type(image_error).__name__)
                await message.reply_text(
                    "⚠️ ตรวจบัญชีสำเร็จ แต่สร้างรูปสรุปไม่สำเร็จครับ"
                )
        else:
...
def start_health_server() -> None:
    # Render Web Service requires the process to listen on the PORT it provides.
...
def main() -> None:
    if not BOT_TOKEN:
        raise SystemExit("กรุณาใส่ TELEGRAM_BOT_TOKEN ใน Environment Variables")
    init_db()
    ...
    # Run a tiny HTTP health server alongside Telegram polling so Render
    # can detect an open port while the bot continues polling Telegram.
