...
async def verify(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        if result.get("success") and result.get("fullname"):
            ...
            # แสดงชื่อโดยตัดคำนำหน้า นาย / นาง / นางสาว
            fullname = re.sub(r"^(นาย|นางสาว|นาง)\s*", "", fullname).strip()
            # แสดงเลขบัญชีเต็มในผลตรวจ
            ...
            # ชื่อและเลขบัญชีใช้ HTML <code> เพื่อให้เป็น Monospace
            result_text = (
                "✅ ตรวจสอบสำเร็จ\n\n"
                f"ธนาคาร: {escape(BANKS.get(bank_code, bank_code))} "
                f"({escape(bank_code)})\n"
