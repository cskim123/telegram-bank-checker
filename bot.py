...
def clean_fullname(fullname: str | None) -> str:
    """Remove Thai/English honorific prefixes from the account name."""
    if not fullname:
        return ""

    name = str(fullname).strip()

    # Remove prefixes repeatedly, including variations such as
    # MR / MR. / Mr. / MRS / MS / MISS / DR and Thai น. น.ส. นส.
    prefix_pattern = re.compile(
        r"^(?:"
        r"นาย|นางสาว|นาง|น\.ส\.|น\.ส|นส\.|นส|น\.|"
        r"mr\.?|mrs\.?|ms\.?|miss\.?|dr\.?"
        r")\s*",
        re.IGNORECASE,
    )
